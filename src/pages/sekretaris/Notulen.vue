<template>
  <div class="space-y-6">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[95vh] overflow-hidden flex flex-col border border-slate-100">
              ...
              <div class="bg-slate-50 rounded-3xl border border-slate-100 p-6">
                <div class="flex flex-col md:flex-row gap-4 mb-4">
                  ...
                  <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase">
                    <span>{{ form.peserta.length }} Terpilih</span>
                    <button type="button" @click="form.peserta = []" class="text-primary-600 hover:underline">Reset</button>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-60 overflow-y-auto pr-2 scrollbar-thin">
                  <label 
                    ...
                    :class="form.peserta.includes(w.nama_kepala_keluarga) 
                      ? 'bg-primary-500 border-primary-500 text-white shadow-lg shadow-primary-500/20' 
                      : 'bg-white border-slate-100 hover:border-primary-200 text-slate-600'"
                  >
                    <input 
                      type="checkbox" 
                      :value="w.nama_kepala_keluarga" 
                      v-model="form.peserta"
                      class="hidden"
                    />
                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all"
                      :class="form.peserta.includes(w.nama_kepala_keluarga) ? 'border-white bg-white' : 'border-slate-200 group-hover:border-primary-400'"
                    >
                      <CheckIcon v-if="form.peserta.includes(w.nama_kepala_keluarga)" class="w-3 h-3 text-primary-500" />
                    </div>
